...
export const MenuProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addBurger = (b: Omit<Burger, "id">) =>
    setBurgers((prev) => [
      ...prev,
      { ...b, id: Math.max(0, ...prev.map((x) => x.id)) + 1 },
    ]);
  ...
  return (
    <MenuContext.Provider
      value={{ burgers, addBurger, updateBurger, deleteBurger }}
    >
      {children}
    </MenuContext.Provider>
  );
